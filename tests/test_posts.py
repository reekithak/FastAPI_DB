...
def test_unauth_del_post(client, test_posts, test_user):
    res = client.delete(f"/posts/{test_posts[0].id}")
    assert res.status_code == 401


def test_auth_del_post(authorized_client, test_posts, test_user):
    res = authorized_client.delete(f"/posts/{test_posts[0].id}")
    assert res.status_code == 204


def test_del_post_non_exist(authorized_client, test_posts, test_user):
    res = authorized_client.delete(f"/posts/42132432234")
    assert res.status_code == 404


def test_del_other_user_post(authorized_client, test_posts, test_user, test_user2):
...
def test_update_nonexist_post(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(f"/posts/122141",
                                json=data)
    assert res.status_code == 404